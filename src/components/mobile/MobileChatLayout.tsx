...
export function MobileChatLayout({ user, roomId, otherUser, allUsers, onBack }: MobileChatLayoutProps) {
    ...
    return (
      <div className="flex h-full">
        <div className="flex-1">
          <ChatArea user={user} roomId={roomId} onMobileBack={handleMobileBack} />
        </div>
          ...
          <div className="w-96">
            ...
            >
              <CollaborationSpace
                ...
                otherUser={otherUser}
                allUsers={allUsers}
                mobileActiveTab={activeTab === "settings" ? "chat" : activeTab}
              />
            </Suspense>
          </div>
        )}
  ...
  const getHeaderTitle = () => {
    switch (activeTab) {
      case "chat":
        return otherUser?.name || `Комната ${roomId}`
      case "games":
        return "Игры"
      case "canvas":
        return "Холст"
    ...
    <div className="flex flex-col h-full bg-black relative overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 text-center">
          <h1 className="text-lg font-bold text-white truncate">{getHeaderTitle()}</h1>
          {activeTab === "chat" && allUsers.length > 0 && (
            <p className="text-xs text-neutral-400">
              {allUsers.length} участник{allUsers.length > 1 ? (allUsers.length < 5 ? "а" : "ов") : ""}
            </p>
          )}
        </div>
        ...
        <button
          ...
          className={cn(
            ...
            isCollabSpaceVisible ? "text-violet-400 bg-violet-400/20" : "text-white hover:bg-white/10",
          )}
      ...
      <UnifiedBottomNav activeTab={activeTab} onTabChange={handleTabChange} unreadCount={0} />
    </div>
  )
}
