...
const choices = [
  { id: 'rock' as const, icon: <HandMetal className="w-8 h-8"/>, color: 'text-gray-400' },
  { id: 'paper' as const, icon: <Hand className="w-8 h-8"/>, color: 'text-white' },
  { id: 'scissors' as const, icon: <Scissors className="w-8 h-8"/>, color: 'text-cyan-400' }
];
            ...
            <CardContent className="flex flex-col items-center gap-6">
                <div className="flex justify-around w-full items-center">
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <div className={`
                            h-24 w-24 text-6xl flex items-center justify-center bg-black/30 rounded-lg 
                            border border-white/10 transition-all duration-300
                            ${isAnimating && selectedChoice ? 'scale-110 ring-2 ring-cyan-400' : ''}
                            ${displayMyMove ? 'bg-cyan-500/20 border-cyan-400/50' : ''}
                        `}>
                            {displayMyMove ? (
                    ...
                    <div className="flex flex-col items-center gap-2">
                        ...
                        <div className={`
                            h-24 w-24 text-6xl flex items-center justify-center bg-black/30 rounded-lg 
                            border border-white/10 transition-all duration-300
                            ${gameState.result && otherMove ? 'bg-purple-500/20 border-purple-400/50' : ''}
                        `}>
                    ...
                    <div className="flex gap-4">
                        {choices.map(choice => (
                            <Button 
                                key={choice.id} 
                                size="icon" 
                                className="h-20 w-20 transition-all hover:scale-110 active:scale-95" 
