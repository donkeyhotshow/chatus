...
export function SharedCanvas({ roomId, sheetId, user, isMazeActive }: SharedCanvasProps) {
  ...
  const gameDocRef = useMemo(() => isMazeActive ? doc(db, 'rooms', roomId, 'games', 'maze') : null, [db, roomId, isMazeActive]);
  const { data: gameState } = useDoc<GameState>(gameDocRef);

  // Subscribe to canvas paths for the current sheet
  const pathsQuery = useMemo(() => query(collection(db, 'rooms', roomId, 'canvasPaths'), where('sheetId', '==', sheetId)), [db, roomId, sheetId]);
  ...
  const checkMazeCollision = (x: number, y: number): boolean => {
    ...
    const maze = JSON.parse(gameState.maze as string);
