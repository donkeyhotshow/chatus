...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Безопасная генерация CSS через useMemo и валидацию
  const styleContent = React.useMemo(() => {
    // Валидация и экранирование id для безопасности
    const safeId = id.replace(/[<>"']/g, '');
    
    return Object.entries(THEMES)
      .map(([theme, prefix]) => {
        // Валидация theme и prefix
        ...
        const colorRules = colorConfig
          .map(([key, itemConfig]) => {
            ...
            // Валидация цвета (только hex или rgb)
            if (!color) return null;
            const safeColor = color.match(/^#[0-9A-Fa-f]{6}$|^rgb\([^)]+\)$/) 
              ? color 
              : '';
