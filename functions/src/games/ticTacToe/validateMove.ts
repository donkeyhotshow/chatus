...
interface TicTacToeState {
  ...
  winner: string | null;
  moveCount: number;
  lastMoveTime?: admin.firestore.FieldValue;
...
export const validateTicTacToeMove = functions.firestore
  .document('rooms/{roomId}/games/{gameId}/moves/{moveId}')
  .onCreate(async (snapshot, context) => {
    const move = snapshot.data() as Move;
    ...
    const game = gameSnap.data() as TicTacToeState;
    ...
    const moveTimestamp = (move.timestamp as admin.firestore.Timestamp)?.toMillis() || Date.now();
    const lastMoveTime = (game.lastMoveTime as admin.firestore.Timestamp)?.toMillis() || (game.createdAt as admin.firestore.Timestamp)?.toMillis() || 0;
...
async function updatePlayerStats(uid: string, result: 'win' | 'loss' | 'draw', gameId: string) {
