...
for p in "${PATTERNS[@]}"; do
  ...
  if grep -R --line-number --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=.github --exclude-dir=.next --exclude-dir=dist --exclude-dir=build --exclude-dir=.vercel -i -n "$p" . >/dev/null 2>&1; then
    echo "Matches for pattern '$p':"
    grep -R --line-number --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=.github --exclude-dir=.next --exclude-dir=dist --exclude-dir=build --exclude-dir=.vercel -i -n "$p" .
    FOUND_ANY=true
  fi
done
...
for p in "${patterns[@]}"; do
	...
	if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
		git grep -n -I -E "$p" -- ':!.github' ':!.next' ':!dist' ':!build' ':!.vercel' || true
	else
		grep -RIn --exclude-dir=.git --exclude-dir=.github --exclude-dir=.next --exclude-dir=dist --exclude-dir=build --exclude-dir=.vercel -E "$p" . || true
	fi
done
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
	if git grep -n -I "BEGIN PRIVATE KEY" -- ':!.github' ':!.next' ':!dist' ':!build' ':!.vercel' || true; then
		found=1
	fi
else
	if grep -RIn --exclude-dir=.git --exclude-dir=.github --exclude-dir=.next --exclude-dir=dist --exclude-dir=build --exclude-dir=.vercel "BEGIN PRIVATE KEY" . || true; then
		found=1
	fi
fi
